...
class CollectGameEnv(MultiGridEnv):
    ...
    def __init__(self, size=10, width=None, height=None, num_balls=[],
                 agents_index = [], balls_index=[], balls_reward=[], zero_sum = False, 
                 partial_obs=False, view_size=7, actions_set=CollectActions, render_mode="rgb_array"
                 ):
        ...
        self.keys = ['agent1ball1', 'agent1ball2', 'agent1ball3', 'agent2ball1', 'agent2ball2', 'agent2ball3']
    ...
    def reset(self, seed=None):
        self.collected_balls = 0
        self.info = {'agent1ball1': 0, 'agent1ball2': 0, 'agent1ball3': 0, 'agent2ball1': 0, 'agent2ball2': 0, 'agent2ball3': 0}
        super().reset()
...
class CollectGame3Obj2Agent(CollectGameEnv):
    ...
    def get_obj_grid(self, ball_idx=[0, 1, 2]):
        ...
        for j in range(self.grid.height):
            for i in range(self.grid.width):
                c = self.grid.get(i, j)
                if c is not None and c.type == 'ball' and self.world.COLOR_TO_IDX[c.color] in ball_idx:
    ...
    def indicator(self):
        ...
        phi_obs = np.zeros((num_layers, self.grid.width, self.grid.height), dtype='uint8')
        for i in range(self.grid.width):
            for j in range(self.grid.height):
                ...
                if obj is None:
                    ...
                    idx = 4 if self.agents_index[0] == (self.world.COLOR_TO_IDX[obj.color]) else 5
                    phi_obs[idx, i, j] = 1
                elif obj.type == 'ball':
